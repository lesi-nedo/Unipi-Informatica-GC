function BuildingMaker(b) {
	...
	this.triangleIndices = new Uint16Array(3 * (2 * nv + nv));

	var triangleOffset = 0;
	for (var i=0; i<nv; ++i) {
		this.triangleIndices[triangleOffset + 0] = (i+1) % nv;
		this.triangleIndices[triangleOffset + 1] = i; // i || nv+ (i-1)%nv;
	...
	for (var i=1; i<(nv-2); ++i) {
		this.triangleIndices[triangleOffset + 0] = (nv*i+1);
		this.triangleIndices[triangleOffset + 1] = i*nv;
		this.triangleIndices[triangleOffset + 2] = nv + (i*nv - 1) ;
		triangleOffset += 3;

		this.triangleIndices[triangleOffset + 0] = (nv*i+1);
		this.triangleIndices[triangleOffset + 1] = nv + (i*nv - 1);
		this.triangleIndices[triangleOffset + 2] = (i*nv  + 2)  ;
		triangleOffset += 3;
	}
